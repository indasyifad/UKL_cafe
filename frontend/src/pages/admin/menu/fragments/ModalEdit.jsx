...
export default function ModalAdd({ isOpen, onClose, payload, reload }) {
  ...
  const submitHandler = async (values) => {
    ...
    menu.nama_menu !== values?.nama_menu && form.append("nama_menu", values?.nama_menu);
    menu.jenis !== values?.jenis && form.append("jenis", values?.jenis);
    menu.harga !== values?.harga && form.append("harga", values?.harga);
    menu.deskripsi !== values?.deskripsi && form.append("deskripsi", values?.deskripsi);
    values?.foto[0] && menu.gambar !== values?.foto[0] && form.append("gambar", values?.foto[0]);
  ...
  return (
    ...
    >
      ...
      <ModalContent borderRadius="20px">
        <ModalBody p={8}>
          <Heading fontSize={20}>Ubah Menu</Heading>
          <Box mt={4}>
