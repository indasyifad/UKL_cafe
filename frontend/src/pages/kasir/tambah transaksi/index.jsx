...
export default function index() {
  ...
  return (
    <Container>
      ...
      >
       <Link to={`/dashboard/kasir/transaksi`}>
        <Button
          colorScheme={"blue"}
          w={"100%"}
          mt={2}
          ...
          >
            <Flex direction="column">
              ...
              >
                {/* kondisi */}
                {menu.map((item) => (
                 kolomMenu.findIndex((x) => x.id_menu === item.id_menu) !== -1 ?
                (<option value={item.nama_menu} disabled>{item.nama_menu}</option>) : (<option value={item.nama_menu}>{item.nama_menu}</option>) 
                ))}
              </Select>
            </Flex>
            <Flex direction="column">
            ...
            <Flex direction="column">
              ...
              <Flex alignItems={"center"} gap={3}>
                ...
                <Button
                  colorScheme={"blue"}
                  size={"md"}
                  onClick={row.jumlah > 0 ? () => {handleChange(row.jumlah - 1, indexRow, "jumlah")} : null}
                >
                  -
                </Button>
